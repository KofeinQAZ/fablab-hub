...
export async function submitAccessRequest({ data }: { data: { type: 'safety_briefing' | 'residency'; description?: string; cv_url?: string; scheduled_time?: string } }) {
  const { type, description, cv_url, scheduled_time } = data;
